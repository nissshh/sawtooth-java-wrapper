 ...
 *
 * @param <ENTITY>
 *            Entity that will be stored on block chaing. The concreate implementation define what
 *            entity they will work on and further defines the adderssing and entities that are
 *            used.
 * @param <PAYLOAD>
...
public abstract class ClientService<ENTITY, PAYLOAD> {
	...
	public Status submitStateChangeMutipleTransactions(List<PAYLOAD> payloads) throws Exception {
	...
	public Status submitStateChangeMutipleBatches(List<PAYLOAD> payloads) throws Exception {
		List<TransactionHeaderDTO> transactionHeaderDTOs = new ArrayList<>();
		TransactionHeaderDTO transactionHeaderDTO = null;
		for (PAYLOAD payload : payloads) {
			transactionHeaderDTO = transactionBuilder.buildTransaction(payload);
	...
	public void setTransactionBuilder(
			GenericTransactionBuilder<ENTITY, PAYLOAD> transactionBuilder) {
