...
public class LoanController {
	...
	@Autowired
	private LoanPayloadClientService loanPayloadService;
	...
	public @ResponseBody String getLoan(@RequestParam String assetId, String lenderId,
			String borrowerId) throws InvalidProtocolBufferException, InterruptedException,
			ValidatorConnectionError, UnsupportedEncodingException {
		Loan entity = Loan.newBuilder().setAssetId(assetId).setBorrowerId(borrowerId)
				.setLenderId(lenderId).build();
	...
	public @ResponseBody int createLoanRequest(@RequestBody CreateLoanVO loanVo) throws Exception {
		CreateLoanRequest creteWallet = CreateLoanRequest.newBuilder()
				...
				.setRequestedAmt(loanVo.getRequestedAmount()).setStatus(LoanStatus.REQUESTED)
				.build();
	...
	public @ResponseBody int approveLoanRequest(@RequestBody ApproveLoanVO loanVo)
			throws Exception {
	...
	public @ResponseBody int loanPayment(@RequestBody LoanPaymentVO loanVo) throws Exception {
		...
		LoanRePaymentPayload loanPaymentPayload = LoanRePaymentPayload.newBuilder()
				.setAssetId(loanVo.getAssetId()).setPayment(payment).build();
