...
public class WalletController {
	...
	public @ResponseBody String getWallet(@RequestParam String customerId) throws InvalidProtocolBufferException, InterruptedException, ValidatorConnectionError, UnsupportedEncodingException {
	...
	public @ResponseBody int putWallet(@RequestBody WalletVO walletVo) throws Exception {
		CreateWallet creteWallet = CreateWallet.newBuilder().setCustomerId(walletVo.getCustomerId).setBalance(walletVo.getBalance()) .build(); 
		SawtoothWalletPayload payload = SawtoothWalletPayload.newBuilder().setPayloadType(PayloadType.CREATE_WALLET).setCreateWallet(creteWallet).build();
	...
	public @ResponseBody int deposit(@RequestBody WalletVO walletVo) throws Exception {
		Deposit depositWallet = Deposit.newBuilder().setCustomerId(walletVo.getCustomerId).setBalance(walletVo.getBalance()) .build(); 
		SawtoothWalletPayload payload = SawtoothWalletPayload.newBuilder().setPayloadType(PayloadType.DEPOSIT).setDeposit(depositWallet).build();
		Status response = walletPayloadService.submitStateChange(payload);
		if(!response.equals(Status.OK) && !response.equals(Status.STATUS_UNSET)) {
			log.warn("Response for submission is : "+ response.getNumber());		
		}
	...
	public @ResponseBody int withdraw(@RequestBody WalletVO walletVo) throws Exception {
		Withdraw withdrawWallet = Withdraw.newBuilder().setCustomerId(walletVo.getCustomerId).setBalance(walletVo.getBalance()) .build(); 
		SawtoothWalletPayload payload = SawtoothWalletPayload.newBuilder().setPayloadType(PayloadType.DEPOSIT).setWithdraw(withdrawWallet).build();
		Status response = walletPayloadService.submitStateChange(payload);
		if(!response.equals(Status.OK) && !response.equals(Status.STATUS_UNSET)) {
			log.warn("Response for submission is : "+ response.getNumber());		
		}
